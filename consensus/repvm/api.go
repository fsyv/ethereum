...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.repvm.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.repvm.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.repvm.lock.RLock()
	defer api.repvm.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.repvm.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.repvm.lock.Lock()
	defer api.repvm.lock.Unlock()

	api.repvm.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.repvm.proposals, address)
}
...
// GetSigner returns the signer for a specific repvm block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.repvm.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.repvm.Author(block.Header())
	}
