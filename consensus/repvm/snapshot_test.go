...
// checkpoint creates a Repvm checkpoint signer section from the provided list
...
// sign calculates a Repvm digital signature for the given block and embeds it
...
// the account may or may not have cast a Repvm vote.
...
// Tests that Repvm signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestRepvm(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Repvm = &params.RepvmConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		...
		// Pass all the headers through repvm and ensure tallying succeeds
